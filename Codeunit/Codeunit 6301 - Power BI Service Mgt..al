codeunit 6301 "Power BI Service Mgt."
{
    ...
    begin
        ...
        ParseReports(TempPowerBIReportBuffer,JObj,Context);
    end;

    procedure GetReportsForUserContext(var TempPowerBIReportBuffer: Record "Power BI Report Buffer" temporary;var ExceptionMessage: Text;var ExceptionDetails: Text;Context: Text[30])
    var
        ...
        if PowerBIReportConfiguration.Find('-') then
          repeat
            if PowerBIReportConfiguration.EmbedUrl <> '' then begin
              ...
              ParseReport(TempPowerBIReportBuffer,JObj,Context);
            end;
          until PowerBIReportConfiguration.Next = 0;
    end;

    procedure CheckForPowerBILicense(var ExceptionMessage: Text;var ExceptionDetails: Text)
    var
    ...
    begin
        ...
        if not PowerBIReportUploads.IsEmpty then
          exit;
        ...
        // This will throw unauthorized error if any. It will swallow 404 not found error.
        GetData(ExceptionMessage,ExceptionDetails,Url);
    end;

    [Scope('Personalization')]
    procedure IsReportEnabled(ReportId: Guid;Context: Text): Boolean
    var
        PowerBIReportConfiguration: Record "Power BI Report Configuration";
    begin
        exit(PowerBIReportConfiguration.Get(UserSecurityId,ReportId,Context));
    end;
    ...
    begin
        ...
        then begin
          ...
          if DotNetExceptionHandler.CastToType(WebException,GetDotNetType(WebException)) then begin
            ...
            if HttpWebResponse.StatusCode = 401 then begin
    ...
    local procedure ParseReport(var TempPowerBIReportBuffer: Record "Power BI Report Buffer" temporary;JObj: DotNet JObject;Context: Text[30])
    var
        PowerBIReportConfiguration: Record "Power BI Report Configuration";
    ...
    local procedure ParseReports(var TempPowerBIReportBuffer: Record "Power BI Report Buffer" temporary;JObj: DotNet JObject;Context: Text[30])
    var
    ...
    local procedure GetReportsUrl(): Text
    begin
        if IsPPE then
    ...
    procedure UpdateEmbedUrlCache(var TempPowerBIReportBuffer: Record "Power BI Report Buffer" temporary;Context: Text)
    var
        PowerBIReportConfiguration: Record "Power BI Report Configuration";
    begin
