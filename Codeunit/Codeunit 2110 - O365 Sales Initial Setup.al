codeunit 2110 "O365 Sales Initial Setup"
{
    ...
    local procedure DisableGraphSync()
    var
        CompanyInformation: Record "Company Information";
        O365SalesBackgroundSetup: Codeunit "O365 Sales Background Setup";
    begin
        if not CompanyInformation.WritePermission then
          exit;

        if not CompanyInformation.Get then
          exit;
        if not CompanyInformation."Sync with O365 Bus. profile" then
          exit;
        ...
        CompanyInformation.Validate("Sync with O365 Bus. profile",false);
        CompanyInformation.Modify(true);
        O365SalesBackgroundSetup.InitializeGraphSync(false,false);
    end;

    local procedure DisableCoupons()
    begin
        if not O365SalesInitialSetup.WritePermission then
          exit;

        if not O365SalesInitialSetup.Get then
          exit;
        if not O365SalesInitialSetup."Coupons Integration Enabled" then
          exit;
        ...
        O365SalesInitialSetup."Coupons Integration Enabled" := false;
        O365SalesInitialSetup.Modify(true);
    end;

    local procedure DisableKPIs()
    var
        O365C2GraphEventSettings: Record "O365 C2Graph Event Settings";
    begin
        if not O365C2GraphEventSettings.WritePermission then
          exit;
