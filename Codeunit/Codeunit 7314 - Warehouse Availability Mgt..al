codeunit 7314 "Warehouse Availability Mgt."
{
    ...
    begin
        ...
        with Item do begin
          ...
          OnAfterCalcReservedQtyOnInventory(Item,ReservedQtyOnInventory);
    ...
    begin
        FilterWhseEntry(WhseEntry,LocationCode,ItemNo,VariantCode,LotNo,SerialNo);
    ...
    [Scope('Personalization')]
    procedure CalcQtyOnAssemblyBin(LocationCode: Code[10];ItemNo: Code[20];VariantCode: Code[10];LotNo: Code[50];SerialNo: Code[50]): Decimal
    var
    ...
    begin
        ...
        WhseEntry.SetRange("Bin Code",Location."To-Assembly Bin Code");
        FilterWhseEntry(WhseEntry,LocationCode,ItemNo,VariantCode,LotNo,SerialNo);
        WhseEntry.CalcSums("Qty. (Base)");
        exit(WhseEntry."Qty. (Base)");
    end;

    procedure CalcQtyOnOpenShopFloorBin(LocationCode: Code[10];ItemNo: Code[20];VariantCode: Code[10];LotNo: Code[50];SerialNo: Code[50]): Decimal
    var
        Location: Record Location;
        WhseEntry: Record "Warehouse Entry";
    begin
        Location.Get(LocationCode);
        if Location."Open Shop Floor Bin Code" = '' then
          exit(0);

        WhseEntry.SetRange("Bin Code",Location."Open Shop Floor Bin Code");
        FilterWhseEntry(WhseEntry,LocationCode,ItemNo,VariantCode,LotNo,SerialNo);
        WhseEntry.CalcSums("Qty. (Base)");
        exit(WhseEntry."Qty. (Base)");
    end;

    procedure CalcQtyOnToProductionBin(LocationCode: Code[10];ItemNo: Code[20];VariantCode: Code[10];LotNo: Code[50];SerialNo: Code[50]): Decimal
    var
        Location: Record Location;
        WhseEntry: Record "Warehouse Entry";
    begin
        ...
        WhseEntry.SetRange("Bin Code",Location."To-Production Bin Code");
        FilterWhseEntry(WhseEntry,LocationCode,ItemNo,VariantCode,LotNo,SerialNo);
        WhseEntry.CalcSums("Qty. (Base)");
        exit(WhseEntry."Qty. (Base)");
    end;
    ...
    local procedure OnAfterCalcReservedQtyOnInventory(var Item: Record Item;var ReservedQtyOnInventory: Decimal)
    begin
    end;

    local procedure FilterWhseEntry(var WhseEntry: Record "Warehouse Entry";LocationCode: Code[10];ItemNo: Code[20];VariantCode: Code[10];LotNo: Code[50];SerialNo: Code[50])
    begin
