codeunit 9510 "Document Service Management"
{
    ...
    procedure CreateFolders(DocFolder: Text)
    var
        DocumentServiceHelper: DotNet NavDocumentServiceHelper;
    begin
        // Tests connectivity to the Document Service using the current configuration in Dynamics NAV.
    ...
    local procedure SetFolderProperties(DocFolder: Text)
    var
        DocumentServiceRec: Record "Document Service";
        DocumentServiceHelper: DotNet NavDocumentServiceHelper;
    begin
        with DocumentServiceRec do begin
          ...
          DocumentService.Properties.SetProperty(FieldName(Password),Password);
          DocumentService.Properties.SetProperty(FieldName("Document Repository"),"Document Repository");
          DocumentService.Properties.SetProperty(FieldName(Folder),DocFolder);
    ...
    local procedure SaveStreamFolder(Stream: InStream;TargetName: Text;Overwrite: Boolean;DocFolder: Text): Text
    var
        DocumentURI: Text;
    begin
        // Saves a stream to the Document Service using the configured location specified in Dynamics NAV.
        CreateFolders(DocFolder);
        SetFolderProperties(DocFolder);
    ...
    procedure SaveFileFolder(SourcePath: Text;TargetName: Text;Overwrite: Boolean;DocFolder: Text): Text
    var
        SourceFile: File;
        SourceStream: InStream;
    begin
        ...
        exit(SaveStreamFolder(SourceStream,TargetName,Overwrite,DocFolder));
    end;
}
