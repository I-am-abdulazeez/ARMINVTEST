codeunit 1259 "Certificate Management"
{
        ...
        with IsolatedCertificate do begin
          if Password <> '' then begin
            if EncryptionManagement.IsEncryptionEnabled then begin
              if not ISOLATEDSTORAGE.Set(Code + PasswordSuffixTxt,Encrypt(Password),GetCertDataScope(IsolatedCertificate)) then
    ...
    begin
        ...
        if EncryptionManagement.IsEncryptionEnabled then
          if TryDecryptPassword(StoredPassword) then;
    ...
    [TryFunction]
    local procedure TryDecryptPassword(var TextToDecrypt: Text)
    begin
        TextToDecrypt := Decrypt(TextToDecrypt);
    end;
}
