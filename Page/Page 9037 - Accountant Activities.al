page 9037 "Accountant Activities"
{
    ...
    layout
    {
        area(content)
        {
            ...
            cuegroup("New Journal Entries")
            {
                Caption = 'New Journal Entries';

                actions
                {
                    action("New G/L Journal Entry")
                    {
                        AccessByPermission = TableData "G/L Entry"=IMD;
                        ApplicationArea = Basic,Suite;
                        Caption = 'New G/L Journal Entry';
                        Image = TileNew;
                        RunObject = Page "General Journal";
                        ToolTip = 'Prepare to post any transaction to the company books.';
                    }
                    action("New Payment Journal Entry")
                    {
                        AccessByPermission = TableData "Gen. Journal Batch"=IMD;
                        ApplicationArea = Basic,Suite;
                        Caption = 'New Payment Journal Entry';
                        Image = TileNew;
                        RunObject = Page "Payment Journal";
                        ToolTip = 'Pay your vendors by filling the payment journal automatically according to payments due, and potentially export all payment to your bank for automatic processing.';
                    }
