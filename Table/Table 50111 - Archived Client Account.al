table 50111 "Archived Client Account"
{
    ...
    fields
    {
        ...
        field(209;"Total Amount Withdrawn";Decimal)
        {
            CalcFormula = Sum("Client Transactions".Amount WHERE ("Account No"=FIELD("Account No"),
                                                                  "Value Date"=FIELD("Date Filter"),
                                                                  "Transaction Type"=CONST(Redemption)));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = Sum("Client Transactions"."Charges On Interest" WHERE ("Account No"=FIELD("Account No")));
            Editable = false;
            FieldClass = FlowField;
        }
    ...
    begin
        if ("Client ID"<>'') and ("Fund No"<>'') then begin
         ...
         Rename("Client ID"+'-'+"Fund No"+'-'+"Fund Sub Account");
