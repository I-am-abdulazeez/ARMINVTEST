table 50087 "Loan Application"
{
    ...
    fields
    {
        ...
        field(14;"On Repayment Holiday";Boolean)
        {
            DataClassification = ToBeClassified;
        }
        ...
        {
            DataClassification = ToBeClassified;
            OptionCaption = 'New,Pending Approval,Rejected,Approved,Partially Repaid,Fully Repaid,Non-Performing,On Repayment Holiday,At Treasury';
            OptionMembers = New,"Pending Approval",Rejected,Approved,"Partially Repaid","Fully Repaid","Non-Performing","On Repayment Holiday","At Treasury";
        }
        ...
        field(24;"Placed On Holiday By";Code[50])
        {
            DataClassification = ToBeClassified;
        }
        field(25;"Date Placed On Holiday";DateTime)
        {
            DataClassification = ToBeClassified;
        }
        ...
        field(58;"Reactivated From Holiday By";Code[50])
        {
            DataClassification = ToBeClassified;
        }
        field(59;"Date Reactivated From Holiday";DateTime)
        {
            DataClassification = ToBeClassified;
        }
        ...
        field(61;"Sent To Treasury";Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(62;"Date Sent To Treasury";DateTime)
        {
            DataClassification = ToBeClassified;
        }
        field(63;"Treasury Batch";Code[10])
        {
            DataClassification = ToBeClassified;
        }
        field(64;"Document Link";Text[250])
        {
            DataClassification = ToBeClassified;
            Editable = false;
            ExtendedDatatype = URL;
        }
        field(65;"Total Principal";Decimal)
        {
            CalcFormula = Sum("Loan Repayment Schedule"."Principal Due" WHERE ("Loan No."=FIELD("Loan No."),
                                                                               "Repayment Date"=FIELD("Date Filter")));
            FieldClass = FlowField;
        }
    ...
    begin
        if "Loan No." = '' then begin
          LoanMgtSetup.Get;
          if LoanMgtSetup."Last Loan App No." <> '' then
          begin
          Evaluate(counters, ConvertStr(LoanMgtSetup."Last Loan App No.",'LN-','000'))
          end;
          counters:= counters+1;
          reccode:= (PadStr('', 16 - StrLen(Format(counters)), '0') + Format(counters));
          "Loan No.":= 'LN-' + reccode;
          LoanMgtSetup.ModifyAll("Last Loan App No.",'LN-' + reccode);
        end;
    ...
    local procedure ValidateLoanApplication()
    var
        LoanApp: Record "Loan Application";
    begin
        LoanApp.Reset;
        LoanApp.SetRange("Client No.","Client No.");
        LoanApp.SetFilter("Principal Repayment Method",'<>%1',LoanApp."Principal Repayment Method"::None);
        if LoanApp.FindFirst then begin
          LoanApp.SetFilter(Status,'<>%1&<>%2&<>%3',LoanApp.Status::"Fully Repaid",LoanApp.Status::Rejected,LoanApp.Status::New);
          if LoanApp.FindFirst then
            Error('There is an active loan on Staff ID %1',"Staff ID")
        end
    end;
}
