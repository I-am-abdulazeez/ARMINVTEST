report 50006 "Sub Accounts Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Sub Accounts Report.rdlc';

    dataset
    {
        dataitem(MasterAccount;Client)
        {
            PrintOnlyIfDetail = true;
            column(Title;MasterAccount.Name)
            {
            }
            dataitem(Client;Client)
            {
                DataItemLink = "Main Account"=FIELD("Membership ID");
                ...
                column(StaffID_Client;Client."Staff ID")
                {
                }
                column(Position_Client;Client.Position)
                {
                }
