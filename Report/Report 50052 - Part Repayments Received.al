report 50052 "Part Repayments Received"
{
    // version THL-LOAN-1.0.0

    DefaultLayout = RDLC;
    RDLCLayout = './Part Repayments Received.rdlc';
        ...
        dataitem("Loan Application";"Loan Application")
        {
            ...
            begin
                if (StartDate <> 0D) and (EndDate <> 0D) then begin
                  ReportPeriod := StrSubstNo('Part repayments For the Period beginning %1 to %2',Format(StartDate,0,4),Format(EndDate,0,4));
                  "Loan Application".SetFilter("Date Filter",'%1..%2',StartDate,EndDate);
                end;
                if (StartDate = 0D) and (EndDate <> 0D) then begin
                ReportPeriod := StrSubstNo('Part repayments from Inception to %1',Format(EndDate,0,4));
                "Loan Application".SetFilter("Date Filter",'..%2',EndDate);
                end;
                if (StartDate <> 0D) and (EndDate = 0D) then begin
                ReportPeriod := StrSubstNo('Part repayments from %1 onwards',Format(StartDate,0,4));
                "Loan Application".SetFilter("Date Filter",'%1..',StartDate);
                end;
                if (StartDate = 0D) and (EndDate = 0D) then
                ReportPeriod := StrSubstNo('Total Part repayments for All Loans',Format(StartDate,0,4));
