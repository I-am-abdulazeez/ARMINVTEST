report 50059 "Repayments Due Schedule"
{
    // version THL-LOAN-1.0.0

    DefaultLayout = RDLC;
    RDLCLayout = './Repayments Due Schedule.rdlc';

    dataset
    {
        dataitem("Loan Application";"Loan Application")
        {
            DataItemTableView = WHERE("Interest Repayment Frequency"=FILTER(<>None),Status=FILTER(<>Rejected),Disbursed=CONST(true));
            ...
            column(ExpectedPaymentDate;ExpectedPaymentDate)
            {
            }
            ...
            begin
                ...
                //Get Expected Repayment Date
                ExpectedPaymentDate := 0D;
                RSchedule.Reset;
                RSchedule.SetRange("Loan No.","Loan Application"."Loan No.");
                RSchedule.SetRange("Repayment Date",StartDate,EndDate);
                if RSchedule.FindFirst then
                  ExpectedPaymentDate := RSchedule."Repayment Date";
                //
                ...
                begin
                  ...
                  LoanBalances[i] := Round(ExpectedRepayment*FMRatios.Percentage/100,0.01)
                  else
                  LoanBalances[i] := 0;
                end;
            end;
            ...
            begin
                if (StartDate <> 0D) and (EndDate <> 0D) then begin
                  ReportPeriod := StrSubstNo('Repayments Schedule Due For the Period beginning %1 to %2',Format(StartDate,0,4),Format(EndDate,0,4));
                  "Loan Application".SetFilter("Date Filter",'%1..%2',StartDate,EndDate);
                end;
                if (StartDate = 0D) and (EndDate <> 0D) then begin
                ReportPeriod := StrSubstNo('Repayments Schedule Due from Inception to %1',Format(EndDate,0,4));
                "Loan Application".SetFilter("Date Filter",'..%2',EndDate);
                end;
                if (StartDate <> 0D) and (EndDate = 0D) then begin
                ReportPeriod := StrSubstNo('Repayments Schedule Due from %1 onwards',Format(StartDate,0,4));
                "Loan Application".SetFilter("Date Filter",'%1..',StartDate);
                end;
                if (StartDate = 0D) and (EndDate = 0D) then
                ReportPeriod := StrSubstNo('Total Repayments Schedule Due on All Loans',Format(StartDate,0,4));
